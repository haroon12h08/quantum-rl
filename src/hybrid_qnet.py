"""
Highly Optimized Quantum DQN for Taxi-v3
Based on latest research (2024-2025):
- RealAmplitudes ansatz (best for discrete actions per EPJ Quantum Tech 2025)
- Dueling architecture (separates state value from action advantages)
- Prioritized experience replay
- He initialization for classical layers
- Optimized hyperparameters from recent benchmarks
"""
...
class OptimizedQuantumDuelingDQN:
    ...
    def __init__(self, n_states=500, n_qubits=4, n_actions=6, n_layers=2):
        ...
        # Quantum circuit - RealAmplitudes ansatz (best for discrete actions)
        ...
        def quantum_circuit(inputs, weights):
            ...
            # RealAmplitudes layers (only RY rotations + entanglement)
        ...
        self.q_weights = pnp.random.uniform(
            -np.pi/4, np.pi/4,  # Shallow initialization
...
def main():
    ...
    agent = OptimizedQuantumDuelingDQN(
        n_states=500,
        n_qubits=4,  # 4 qubits optimal per research
        n_actions=6,
        n_layers=2  # Shallow circuits work best
    )
    ...
    # Optimized hyperparameters from 2024-2025 research
    optimizer = qml.AdamOptimizer(stepsize=5e-4)  # Lower LR for quantum
    
    epochs = 100
    episodes_per_epoch = 25
    batch_size = 64
    gamma = 0.99  # Higher gamma for Taxi
    epsilon_start = 1.0
    epsilon_end = 0.01
    epsilon_decay = 0.996
    ...
    print("Training Optimized Quantum Dueling DQN (RealAmplitudes)")
    print("=" * 70)
    ...
    for epoch in range(epochs):
        ...
        if total_episodes >= warmup_episodes and len(replay_buffer) >= batch_size:
            n_updates = 10  # Multiple updates per epoch
            
            for _ in range(n_updates):
                states, actions, rewards, next_states, dones, indices, weights = replay_buffer.sample(batch_size)
                
                # Loss function with 16 parameters (embeddings, w_proj, b_proj, q_weights, v_w, v_b, 6 adv heads)
                ...
                # Compute TD errors for priority update (after parameter update)
                td_errors = []
                for i in range(len(states)):
    ...
    print(f"  Average reward:  {np.mean(test_rewards):6.2f} ± {np.std(test_rewards):.2f}")
    print(f"  Median reward:   {np.median(test_rewards):6.2f}")
    print(f"  Average steps:   {np.mean(test_steps):6.1f}")
    print(f"  Success rate:    {successes}/50 ({successes/50*100:.0f}%)")
    print(f"  Best reward:     {np.max(test_rewards):6.1f}")
    print(f"  Worst reward:    {np.min(test_rewards):6.1f}")
    test_env.close()
